...
STRATEGY_GRAMMAR_PROMPT = (
    ...
    '- {"op":"indicator","kind":"candlestick","pattern":"<name>"}              // +100 bullish / -100 bearish / 0 none\n'
    '    pattern is one of: engulfing, hammer, invertedHammer, shootingStar, hangingMan, doji, morningStar,\n'
    '    eveningStar, harami, piercing, darkCloudCover, threeWhiteSoldiers, threeBlackCrows.\n'
    '    Use as a reversal SIGNAL: "candlestick engulfing > 0" = bullish engulfing this bar; "< 0" = bearish.\n'
    '    CONFIRM IT — a raw candlestick pattern is a NOISY, low-reliability signal and you must NEVER enter\n'
    '    on the pattern alone. AND it with at least one confirmation: a trend filter (close vs a 100/200 EMA),\n'
    '    an oscillator (RSI/Stochastic oversold for a bullish pattern, overbought for a bearish one), or a\n'
    '    volume spike (price.volume > sma of volume). Unconfirmed pattern entries whipsaw and fail the gate.\n'
    '\n'
...
_DIRECTION_FUTURES = (
    ...
    'MIRROR THE LEVELS, AND KEEP THEM TRADEABLE. Two rules, both enforced:\n'
    '  (a) the long and short oscillator entry levels MUST sum to 100. A lopsided pair such as 30/60 '
    'is REJECTED — it makes one leg fire far more often, so the strategy is one-sided while claiming '
    'to trade both.\n'
    '  (b) the LONG entry level must be between 35 and 45, and the SHORT between 55 and 65. Use '
    '35/65 or 40/60. Extremes like 30/70 are REJECTED even though they mirror: an RSI below 30 while '
    'price is ABOVE the trend EMA almost never happens, and such a strategy produced just two long '
    'entries in seven months. Moderate levels are what make BOTH legs actually trade.\n'
