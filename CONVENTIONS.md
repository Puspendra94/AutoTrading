...
- **Background jobs**: live in `apps/backend/src/worker.ts` (separate process, `NestFactory.createApplicationContext`, no HTTP listener), using `@nestjs/schedule` `@Cron`/`@Interval` decorators calling the same injectable services the API process uses. The API process (`main.ts`) never runs scheduled jobs itself, and the worker never listens on HTTP.
...
- Config values called out in spec Section 12 (risk limits, probation sizing, sync frequencies, etc.) are `.env`-driven with sane defaults, never magic numbers buried in a service.
